...
const enterProgramDetails = async (req, res) => {
  try {
    ...
    foundUser.program.push(savedProgram._id);
    const result = await foundUser.save();

    if (!result) {
      res.status(500).json({
        error: "An error occurred while saving the program to the user",
      });
    }
...
const editProgramDetails = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "An error occurred while  updating the program." });
  }
}; //TESTED
